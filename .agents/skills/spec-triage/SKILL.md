...
| Requires infra provisioning (new GCP service, new table, new secret)? | `BACKLOG.md` |
...
- New functional requirement → `## 4. Functional Requirements`, next `FRN+1`
- New business rule → `## 5. Business Rules`, next `BRN+1`
- New non-functional requirement → `## 6. Non-Functional Requirements`, next `NFRN+1`
...
Additive CLI flag, no schema change, touches 1–2 files → `SPEC.md` as `FR29`.
