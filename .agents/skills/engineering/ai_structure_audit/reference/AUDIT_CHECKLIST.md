...
#### 3c. Flag Anomalies
- Flag unexpected files, empty directories, or missing expected files (e.g. skill directory lacking `SKILL.md` or files not listed in `STRUCTURE.md`).
...
- Verify every file mentioned in `STRUCTURE.md` index exists.

#### 5b. Reference Validity
- Verify every file referencing another (e.g. `WORKFLOW.md` referencing `SPEC.md`) points to real, populated content. Check for missing files.
...
- Verify task phases in `TASKS.md` align with methodology declared in `AGENTS.md`.

#### 7d. Dependency Governance
- Verify approved dependencies in `HARNESS.md` are concrete decisions, not option lists.
...
- Check for domain leakage between `WORKFLOW.md`, `HARNESS.md`, and `SPEC.md`.
...
#### 11b. Skill-to-Task Ratio
- Check pre-computed ratio of skills to active tasks in `TASKS.md`.
...
- Metrics: Total files, Skills, Active tasks, Skill-to-task ratio, Dead files (zero refs), Max hop depth.
