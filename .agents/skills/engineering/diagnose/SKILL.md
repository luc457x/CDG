---
name: diagnose
description: Disciplined diagnosis loop for hard bugs and performance regressions. Reproduce → minimise → hypothesise → instrument → fix → regression-test. Use when user says "diagnose this" / "debug this", reports bug, says something broken/throwing/failing, or describes performance regression.
---
...
**Then ask: what would have prevented this bug?** If answer involves architectural change (no good test seam, tangled callers, hidden coupling) hand off to `/improve_codebase_architecture` skill with specifics. Make recommendation **after** fix is in, not before — have more info now.
