...
def analyze_skill_file(skill_path, skills_dir, max_lines=180):
    ...
    desc_len = len(desc)
    has_use_when = "use when" in desc.lower() or "triggers" in desc.lower() or "when" in desc.lower()
...
def main():
    ...
    for is_main, r in sorted(results, key=lambda x: (x[1]["name"], x[1]["path"])):
        ...
        if r["has_frontmatter"] and not r["has_use_when"]:
            issues.append("Description lacks 'Use when...' trigger phrase")
        if r["desc_len"] > 300:
            has_detailed_use_section = any(any(q in s[1].lower() for q in ["when to use", "quick start", "philosophy", "glossary"]) for s in r["sections"])
            if not has_detailed_use_section:
                issues.append(f"Description length ({r['desc_len']}) exceeds 300 characters. Extract detailed triggers to a 'When to Use' body section and keep frontmatter description concise (under 300 chars).")
            else:
                issues.append(f"Description length ({r['desc_len']}) exceeds 300 characters. Frontmatter description must be concise (under 300 chars) even if a 'When to Use' body section is present.")
        ...
        # Enforce caveman style: no articles in YAML description, and prose ratio <= 2.0%
