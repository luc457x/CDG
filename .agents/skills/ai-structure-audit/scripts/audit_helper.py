...
def style_consistency_check(inventory):
    ...
    def is_numbered_doc(path):
        return os.path.basename(path) in ('SPEC.md',)
    ...
    # Emphasis overuse (raise threshold to 20%; ignore files that are indexes or lists)
    overused = []
    for p, v in emphasis_ratio.items():
        if v > 0.20:
            if os.path.basename(p) not in ('PROGRESS.md',):
                overused.append(p)
...
def run_audit(workspace_root=None, output_path=None):
    ...
    depended_upon = ["SPEC.md"]
    # Optional files (e.g. UBIQUITOUS_LANGUAGE.md) are not treated as critical dependencies
    # unless explicitly required by project rules.
    ...
    for root, dirs, files in os.walk(agents_dir):
        for file in files:
            if file.endswith('.md') or file.endswith('.ps1') or file.endswith('.sh'):
                ...
                elif file == "rules/engineering.md":
                    purpose = "Technical harness & QA rules"
                elif file == "rules/workflow.md":
                    purpose = "Workflow rules"
                elif file == "rules/structure.md":
                    purpose = "Documentation structure rules"
