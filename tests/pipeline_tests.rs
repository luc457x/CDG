...
fn test_backtest_with_risk_free_rate() {
    ...
    let (res_rf, _, _) = cdg::backtest::run_backtest_for_asset(&df, "asset", "rsi", 0.0, 0.0, 252.0, &mut None).unwrap();
    ...
    let (res_no_rf, _, _) = cdg::backtest::run_backtest_for_asset(&df_no_rf, "asset", "rsi", 0.0, 0.0, 252.0, &mut None).unwrap();
